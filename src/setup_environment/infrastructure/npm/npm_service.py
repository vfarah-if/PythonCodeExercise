...
class NPMFileService(NPMService):
    ...
    def prompt_for_token(self) -> PersonalAccessToken:
        ...
        click.echo("\n" + "=" * 70)
        click.echo(click.style("🔐 GitHub Personal Access Token Setup", bold=True, fg="cyan"))
        click.echo("=" * 70)
        
        if not self.config_exists():
            click.echo(click.style("\n⚠️  No ~/.npmrc file found", fg="yellow"))
            click.echo("We'll create one for you with the necessary GitHub Package Registry settings.")
        ...
        click.echo(click.style("\n   Token Name:", bold=True) + " npm-packages (or any descriptive name)")
        click.echo(click.style("   Expiration:", bold=True) + " 90 days (recommended) or custom")
        click.echo(click.style("\n   Required Permissions:", bold=True, fg="green"))
        click.echo("   ☐ " + click.style("repo", bold=True) + " - Full control of private repositories")
        click.echo("   ☐ " + click.style("write:packages", bold=True) + " - Upload packages to GitHub Package Registry")
        click.echo("   ☐ " + click.style("read:packages", bold=True) + " - Download packages from GitHub Package Registry")
        click.echo("   ☐ " + click.style("delete:packages", bold=True) + " - Delete packages from GitHub Package Registry")
        
        click.echo(click.style("\n⚠️  IMPORTANT:", bold=True, fg="yellow"))
        click.echo(click.style("   GitHub will only show your token ONCE!", fg="yellow"))
        ...
        try:
            ...
            click.echo(click.style(f"⚠️  Could not open browser automatically: {e}", fg="yellow"))
            click.echo(f"Please manually open: {token_url}")
        
        click.echo("\n" + "=" * 70)
        click.echo(click.style("Waiting for you to create and copy your token...", fg="cyan"))
        click.echo("=" * 70)
        ...
        while True:
            ...
            if not token_input:
                click.echo(click.style("✗ Token cannot be empty. Please try again.", fg="red"))
                continue
            
            try:
                token = PersonalAccessToken(token_input)
                click.echo(click.style(f"\n✓ Token validated successfully: {token}", fg="green", bold=True))
                click.echo(click.style("Your token has been securely stored.", fg="green"))
                return token
            except ValueError as e:
                click.echo(click.style(f"\n✗ Invalid token format: {e}", fg="red"))
                click.echo("GitHub tokens should start with 'ghp_' (classic) or 'github_pat_' (fine-grained)")
                retry = click.confirm("Would you like to try again?", default=True)
                if not retry:
                    raise ValueError("Token creation cancelled by user")